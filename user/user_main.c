...
void sensor_timerfunc(void *arg){
	uint16_t humidity;
	uint16_t temp;
	ets_uart_printf("Start process\r\n");

	uint8_t res = am2320_read(&humidity, &temp);
	if (res == RES_OK){
		ets_uart_printf("Result: humid: %d.%d, temp: %d.%d\r\n", humidity/10, humidity % 10, temp/10, temp % 10);
	} else if (res == ERR_CRC){
		ets_uart_printf("Error reading - CRC\r\n");
	} else {
		ets_uart_printf("Error reading - general\r\n");
	}
}
...
//    if (0) {
    	ets_uart_printf("AM2320 init done.\r\n");
        //Disarm timer
